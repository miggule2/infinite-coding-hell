...
def outputWeights(weights, path):
    ...
    for f, v in sorted(weights.items(), key=lambda x: -x[1]):
...
def verbosePredict(phi, y, weights, out):
    ...
    for f, v in sorted(phi.items(), key=lambda x: -x[1] * weights.get(x[0], 0)):
        w = weights.get(f, 0)
...
def outputClusters(path, examples, centers, assignments):
    ...
    for j in range(len(centers)):
        ...
        for k, v in sorted(centers[j].items(), key = lambda x: -x[1]):
            if v != 0:
